...
from ..utils.reward import MultiTargetVelocityReward
...
class BaseHumanoid4Ages(BaseHumanoid):
    ...
    def __init__(self, scaling=None, scaling_trajectory_map=None, use_muscles=False,
        ...
        # if use_muscles:
        #     xml_path = (Path(__file__).resolve().parent / "data" / "humanoid" /
        ...
        if scaling is None:
            ...
            if type(scaling) == list:
                ...
                self._scalings = [scaling]
        # Non-linear estimation of the hips from 0 (considering the subject size) to the approximated floor contact
        # "infant": -0.34,
        # "boy": -0.434,
        # "teen": -0.496,
        # "man": -0.62
        self.has_fallen_scale = {"1": -0.34, "2": -0.434, "3": -0.496, "4": -0.62, "all": -0.62}
    ...
    def load_trajectory(self, traj_params, scaling_trajectory_map=None, warn=True):
        ...
        if scaling_trajectory_map is None:
            if self._scaling_trajectory_map is None and type(self._scalings) is list and len(self._scalings) > 1:
                if warn:
                    warnings.warn(
                        "\"scaling_trajectory_map\" is not defined! Loading the default map, which assumes that "
    ...
    def _create_observation(self, obs):
        ...
        if self.more_than_one_env:
            ...
            obs = np.concatenate([obs[:-len(self.obs_names)], env_id_map, obs[-len(self.obs_names):]])
        return obs
    ...
    def scale_body(xml_handle, scaling, use_muscles):
        ...
        car_parts = ["wheel", "chassis", "spoiler"]
        new_mesh_handle = []
        for h in mesh_handle:
            if len(re.findall(r"(?=(" + '|'.join(car_parts) + r"))", h.name)) < 1:
                new_mesh_handle.append(h)
        ...
        for h in geoms:
            if h.name in head_geoms:  # change position of head
                # scaling values are = [0.4, 0.6, 0.8, 1.0]
                # original implementation: -0.5 * (1 - scaling)
                h.pos = [0.0, - 0.470478 * (scaling) - 0.5 * (1 - scaling), 0.0]  # verify it.

        body_handle = xml_handle.find_all("body")
        body_handle = body_handle[9:]  # removing car bodys
    ...
    def generate(env, task="walk", mode="all", dataset_type="real", n_models=None, debug=False, **kwargs):
        ...
        Args:
            env (class): Humanoid class, either HumanCollisionTorque4Ages or HumanCollisionMuscle4Ages.
        ...
        if task == "walk":
            ...
            local_path = f"datasets/humanoids/{dataset_type}/02-constspeed_reduced_humanoid_POMDP" + dataset_suffix
            ...
            local_path = f"datasets/humanoids/{dataset_type}/05-run_reduced_humanoid_POMDP" + dataset_suffix
            use_mini_dataset = not os.path.exists(Path(loco_mujoco.__file__).resolve().parent / local_path)
            if debug or use_mini_dataset:
                if use_mini_dataset:
                    warnings.warn("Datasets not found, falling back to test datasets. Please download and install "
