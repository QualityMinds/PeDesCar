...
from ..base import ValidTaskConf
from ..utils import check_validity_task_mode_dataset
...
class BaseHumanoid(LocoEnv):
    ...
    def __init__(self, use_muscles=False, use_box_feet=True, disable_arms=True, alpha_box_feet=0.5, **kwargs):
        ...
        """
        if use_muscles:
            if "autocar" in kwargs["car_model"]:
                xml_path = (Path(__file__).resolve().parent.parent.parent / "impact_collision_m1.xml").as_posix()
            elif "prius" in kwargs["car_model"]:
                xml_path = (Path(__file__).resolve().parent.parent.parent / "impact_collision_m2.xml").as_posix()
            else:
                raise ValueError(
                    f'the selected mode {kwargs["car_model"]} is not valid. Please select: autocar, prius')
        else:
            if "autocar" in kwargs["car_model"]:
                xml_path = (Path(__file__).resolve().parent.parent.parent / "impact_collision_m3.xml").as_posix()
            elif "prius" in kwargs["car_model"]:
                xml_path = (Path(__file__).resolve().parent.parent.parent / "impact_collision_m4.xml").as_posix()
            else:
                raise ValueError(
                    f'the selected mode {kwargs["car_model"]} is not valid. Please select: autocar, prius')
        # xml_path = (Path(__file__).resolve().parent.parent / "data" / "humanoid" / "humanoid_torque.xml").as_posix()
        # print(f'Edited... Using model located in the follwoing path: \n{xml_path}')
        del kwargs["car_model"]

        action_spec, detailed_actions = self._get_action_specification(use_muscles)
        self.car_actions_names = detailed_actions["car"]
        observation_spec, detailed_obs = self._get_observation_specification()
        self.obs_names = detailed_obs["car"]
    ...
    def _get_xml_modifications(self):
        ...
        if self._use_box_feet:
            ...
            if not self._use_muscles:
                motors_to_remove += ["mot_subtalar_angle_l", "mot_mtp_angle_l", "mot_subtalar_angle_r",
                                     "mot_mtp_angle_r"]
    ...
    def _has_fallen(self, obs, return_err_msg=False):
        ...
        pelvis_height_condition1 = False # obs[2] < self.height_bound ## not solved =(
        pelvis_height_condition2 = self._data.body("pelvis_body").xpos[2] < 0.12
        pelvis_condition = pelvis_height_condition1 or pelvis_height_condition2

        lumbar_condition = False
    ...
    def generate(env, path, task="walk", dataset_type="real", debug=False, **kwargs):
        ...
        if task == "walk":
            path = f"datasets/humanoids/{dataset_type}/02-constspeed_reduced_humanoid.npz"
            ...
            path = f"datasets/humanoids/{dataset_type}/05-run_reduced_humanoid.npz"
            use_mini_dataset = not os.path.exists(Path(loco_mujoco.__file__).resolve().parent / path)
            if debug or use_mini_dataset:
                if use_mini_dataset:
                    warnings.warn("Datasets not found, falling back to test datasets. Please download and install "
    ...
    def _get_observation_specification():
        ...
        human_observation_spec = [  # ------------- JOINT POS -------------
        ...
        car_observation_spec = [
            # ------------- JOINT POS -------------
            ("q_car_front_left_wheel_joint", "front_left_wheel_joint", ObservationType.JOINT_POS),
            ("q_car_front_right_wheel_joint", "front_right_wheel_joint", ObservationType.JOINT_POS),
            ("q_car_rear_left_wheel_joint", "rear_left_wheel_joint", ObservationType.JOINT_POS),
            ("q_car_rear_right_wheel_joint", "rear_right_wheel_joint", ObservationType.JOINT_POS),
            ("q_car_front_left_steer_joint", "front_left_steer_joint", ObservationType.JOINT_POS),
            ("q_car_front_right_steer_joint", "front_right_steer_joint", ObservationType.JOINT_POS),
            ("q_car_steering_joint", "steering_joint", ObservationType.JOINT_POS),
            # ------------- JOINT VEL -------------
            ("dq_car_front_left_wheel_joint", "front_left_wheel_joint", ObservationType.JOINT_POS),
            ("dq_car_front_right_wheel_joint", "front_right_wheel_joint", ObservationType.JOINT_VEL),
            ("dq_car_rear_left_wheel_joint", "rear_left_wheel_joint", ObservationType.JOINT_VEL),
            ("dq_car_rear_right_wheel_joint", "rear_right_wheel_joint", ObservationType.JOINT_VEL),
            ("dq_car_front_left_steer_joint", "front_left_steer_joint", ObservationType.JOINT_VEL),
            ("dq_car_front_right_steer_joint", "front_right_steer_joint", ObservationType.JOINT_VEL),
            ("dq_car_steering_joint", "steering_joint", ObservationType.JOINT_VEL),
        ]
        observation_spec = []
        observation_spec.extend(human_observation_spec)
        observation_spec.extend(car_observation_spec)
        return observation_spec, {"human": human_observation_spec,
                                  "car": car_observation_spec, }
    ...
    def _get_action_specification(use_muscles):
        ...
        """
        if use_muscles:
            human_action_spec = ["mot_shoulder_flex_r", "mot_shoulder_add_r", "mot_shoulder_rot_r", "mot_elbow_flex_r",
                                 "mot_pro_sup_r", "mot_wrist_flex_r", "mot_wrist_dev_r", "mot_shoulder_flex_l",
                                 "mot_shoulder_add_l", "mot_shoulder_rot_l", "mot_elbow_flex_l", "mot_pro_sup_l",
                                 ...
                                 "bifemlh_r", "bifemsh_r", "sar_r", "add_long_r", "add_brev_r", "add_mag1_r",
                                 "add_mag2_r",
            ...
            human_action_spec = ["mot_lumbar_ext", "mot_lumbar_bend", "mot_lumbar_rot", "mot_shoulder_flex_r",
        ...
        car_action_spec = ["front_wheel", "back_wheel", "turn", ]
        action_spec = []
        action_spec.extend(human_action_spec)
        action_spec.extend(car_action_spec)
        return action_spec, {"human": human_action_spec,
                             "car": car_action_spec, }
